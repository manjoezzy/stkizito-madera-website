...
const NAV_ITEMS: { section: Section; label: string; icon: React.ElementType }[] = [
  ...
  { section: 'events', label: 'News & Events', icon: Calendar },
...
export default function AdminDashboard() {
  ...
  const toggleEventPublish = async (event: EventItem) => {
    try {
  ...
  const [editEventForm, setEditEventForm] = useState({
    title: '', description: '', category: 'general', eventDate: '', eventTime: '', location: '', isBanner: false, isPublished: true,
  });
  ...
  const saveEditEvent = async () => {
    ...
    try {
      const res = await fetch('/api/events', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editingEvent.id, ...editEventForm }),
      });
      const json = await res.json();
      if (json.success) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex items-start">
        ...
        <DialogContent className="sm:max-w-lg max-h-[85vh]">
          ...
          <div className="space-y-3 overflow-y-auto -mx-2 px-2">
            ...
            <div>
              ...
              <textarea
                className="w-full min-h-[100px] rounded-md border border-slate-200 p-2 text-sm resize-y"
                placeholder="Full description or news article content..."
      ...
      <Dialog open={!!editingEvent} onOpenChange={(open) => { if (!open) setEditingEvent(null); }}>
        <DialogContent className="sm:max-w-lg max-h-[85vh]">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-[#1a3a6b]">Edit News / Event</DialogTitle>
            <DialogDescription>Update the details below.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 overflow-y-auto -mx-2 px-2">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Title *</label>
              <Input placeholder="News or event title" value={editEventForm.title} onChange={(e) => setEditEventForm({ ...editEventForm, title: e.target.value })} />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Description</label>
              <textarea className="w-full min-h-[80px] rounded-md border border-slate-200 p-2 text-sm" placeholder="Full description..." value={editEventForm.description} onChange={(e) => setEditEventForm({ ...editEventForm, description: e.target.value })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Date</label>
                <Input type="date" className="h-9 text-sm" value={editEventForm.eventDate} onChange={(e) => setEditEventForm({ ...editEventForm, eventDate: e.target.value })} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Time</label>
                <Input type="time" className="h-9 text-sm" value={editEventForm.eventTime} onChange={(e) => setEditEventForm({ ...editEventForm, eventTime: e.target.value })} />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-700 mb-1">Location</label>
                <Input className="h-9 text-sm" placeholder="e.g. Main Hall" value={editEventForm.location} onChange={(e) => setEditEventForm({ ...editEventForm, location: e.target.value })} />
              </div>
            </div>
            <div className="flex items-center gap-3 pt-2">
              <Button onClick={saveEditEvent} className="flex-1 bg-[#1a3a6b] hover:bg-[#2756a0] text-white">
                Save Changes
              </Button>
...
function EventsSection({ events, onCreateNew, onDelete, onTogglePublish, onEdit, formatDate }: EventsProps) {
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-colors ${event.isPublished ? 'bg-emerald-500 text-white' : 'bg-slate-300 text-white'}`}
                title={event.isPublished ? 'Published (click to unpublish)' : 'Draft (click to publish)'}
              >
                {event.isPublished ? 'P' : 'D'}
              </button>
              {event.attachmentUrl && (
                <a
            ...
            <div className="flex items-center gap-0.5">
              <button onClick={() => onEdit(event)} className="h-7 w-7 flex items-center justify-center text-slate-400 hover:text-[#1a3a6b] transition-colors" title="Edit">
    ...
    <div className="space-y-6">
      ...
      {events.length === 0 ? (
        <div className="py-16 text-center">
          ...
          <p className="text-slate-400 font-medium">No news or events yet</p>
          <p className="text-slate-300 text-sm mt-1">Create your first news article or event</p>
        </div>
      ) : (
        <>
